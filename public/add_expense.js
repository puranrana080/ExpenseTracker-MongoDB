...
async function updatePremiumStatus() {
  try {
    ...
    const userResponse = await axios.get(
      "http://localhost:3000/user/ispremium",
      { headers: { Authorization: token } }
    );
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const downloadlistresponse = await axios.get(
      "http://localhost:3000/user/downloadlist",
      { headers: { Authorization: token } }
    );
...
function getExpenses(page) {
  ...
  axios
    .get(
      `http://localhost:3000/expense/get-expense?page=${page}&limit=${itemsPerPage}`,
      {
        headers: { Authorization: token },
      }
    )
...
function displayDownloadList(downloadList, i) {
  ...
  fileList.appendChild(
    document.createTextNode(
      `File ${i + 1}:-  ${downloadList.fileDownloadedURL}  `
    )
  );
...
function displayExpenseOnScreen(expense) {
  ...
  delBtn.addEventListener("click", () => {
    ...
    axios
      .delete(`http://localhost:3000/expense/delete-expense/${expense._id}`, {
        headers: { Authorization: token },
      })
      .then((result) => {
